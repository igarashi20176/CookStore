...
<label for="my-modal-4" class="modal cursor-pointer">
  <label class="modal-box relative w-11/12 max-w-5xl" for="">
    ...
    <div class="mt-10">
      ...
      <select v-model="select_values.category" required class="hidden peer-checked/meat:block mt-5 m-auto select select-accent border-red-400 w-full max-w-xs">
      ...
      <select v-model="select_values.category" class="hidden peer-checked/vegetable:block mt-5 m-auto select select-accent border-green-400 w-full max-w-xs">
      ...
      <select v-model="select_values.category" required class="hidden peer-checked/milk:block mt-5 m-auto select select-accent border-blue-400 w-full max-w-xs">
      ...
      <select v-model="select_values.category" class="hidden peer-checked/process:block mt-5 m-auto select select-accent border-orange-400 w-full max-w-xs">
      ...
      <input id="volume" required v-model="select_values.amount" type="text" placeholder="例)100g, 小さじ1, 少々" class="mt-6 mr-5 input input-bordered input-accent max-w-xs" />
      ...
      <input id="grams" v-model="select_values.grams" type="text" placeholder="※数字のみ入力してください" class="disabled mt-2 input input-bordered input-accent max-w-xs" :disabled="!props.isActive" />

      <button  @click.prevent.stop="add_ingredients" class="ml-4 mb-5 btn btn-success">追加</button>
      
      <div>
        <p class="font-bold mb-1">★ 選択した食材/分量</p>
        <ul v-if="!(ingredients.length === 0)" v-for="ingredient in ingredients">
          <li>{{ ingredient }}</li>
        </ul>
      </div>  
...
const select_values = ref<select>({
  category: "",
  amount: "",
  grams: 0
});
